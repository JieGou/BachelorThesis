    ...
    class PrioritizedPlanner : ConstraintSolver
    {
        ...
        public override Tour[][] FindTours()
        {
            ...
            foreach (var (tour, agent) in heuristicEnum)
            {
                ...
                while (true)
                {
                    solution[agent][tour] = solver.SolveGTSP(graph, constraints, instance.orders[agent][tour], offsetTime);
            ...
            foreach (var itemRoutes in solution)
            {
                foreach (var itemRoute in itemRoutes)
                {
                    itemRoute.routePaths = GetRoutePaths(graph, itemRoute);
                }
            }
            return solution;
        }
        ...
        /// <param name="route">拣货路径</param>
        /// <returns></returns>
        private List<List<Point>> GetRoutePaths(Graph graph, Tour route)
        {
            var rst = new List<List<Point>>();
            foreach (var itemRoutes in route.routes)
            {
                var itemPath = new List<Point>();
                foreach (var item in itemRoutes)
                {
                    var vertex = graph.vertices.FirstOrDefault(v => v.index == item);
                    if (vertex != null) itemPath.Add(vertex.coordinates);
                }
                rst.Add(itemPath);
            }
            return rst;
